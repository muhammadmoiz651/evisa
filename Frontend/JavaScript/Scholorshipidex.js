async function fetchScholarshipData() {
  try {
    const response = await fetch('/scholarshipsapi?id=1'); // Change the ID as needed
    if (!response.ok) {
    ...
    if (data && Array.isArray(data) && data.length > 0) {
      const scholarship = data[0]; // Get the first item in case it's an array
      ...
      const applyLink = 'https://www.universite-paris-saclay.fr/en/admission/bourses-et-aides-financieres/international-masters-scholarships-program-idex'; 
      const applyNowElement = document.getElementById('apply-now');
      applyNowElement.setAttribute('href', applyLink);
      applyNowElement.setAttribute('target', '_blank'); // Open link in a new tab
    } else if (data && typeof data === 'object') {
      // If the data is a single object (not in an array)
      const scholarship = data; // Use the data directly
      
      document.getElementById('scholarship-title').textContent = scholarship.title;
      document.getElementById('scholarship-subtitle').textContent = scholarship.degrees + ' - ' + scholarship.type;
      document.getElementById('deadline-date').textContent = new Date(scholarship.deadline).toLocaleDateString();
      document.getElementById('apply-now').setAttribute('href', scholarship.apply_link);
    } else {
