const animatedSections = document.querySelectorAll('.animated-top');

const observer = new IntersectionObserver(entries => {
  entries.forEach(entry => {
    if (entry.isIntersecting) {
      entry.target.style.animationDelay = "0.1s"; // optional delay
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  fetch('/api/details')
    ...
    .then((data) => {
      if (data.length > 0) {
        const university = data[2]; // Assuming you want the first university for demonstration
  ...
  <span class="stat-label">
    <a 
      href="https://www.tum.de/en/news-and-events/all-news/press-releases/details/tum-ranks-28th-worldwide" 
      target="_blank" 
...
`;
        ...
        const undergradPrograms = programs.slice(0, 4);
        const masterPrograms = programs.slice(4);
