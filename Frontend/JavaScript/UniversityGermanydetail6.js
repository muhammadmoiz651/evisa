document.addEventListener("DOMContentLoaded", function () {
  ...
  fetch('/api/details')
    ...
    .then((data) => {
      if (data.length > 0) {
        const university = data[15]; // Assuming you want the first university for demonstration
  ...
  <span class="stat-label">
    <a 
      href="https://www.adscientificindex.com/university/Hochschule+M%C3%BCnchen/#:~:text=Hochschule%20M%C3%BCnchen%20%3A%20Annual%20Academic%20Performance%20Analysis" 
      target="_blank" 
...
`;
        ...
        const undergradPrograms = programs.slice(0, 9);
        const masterPrograms = programs.slice(9);
