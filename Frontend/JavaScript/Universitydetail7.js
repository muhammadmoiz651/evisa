document.addEventListener("DOMContentLoaded", function () {
  ...
  fetch('/api/details')
    ...
    .then((data) => {
      if (data.length > 0) {
        const university = data[0]; // Assuming you want the first university for demonstration
  ...
  <span class="stat-label">
    <a 
      href="https://www.topuniversities.com/universities/ens-paris-saclay#:~:text=ENS%20Paris%2DSaclay%20is%20one,QS%20World%20University%20Rankings%202021." 
      target="_blank" 
  ...
  <div class="accordion-group">
    ...
    <div class="accordion-header" id="polyHeading">
      Ingénieur Polytechnicien Program
      <span class="icon" id="polyIcon">+</span>
    </div>
    <div id="polyContainer" class="program-list" style="display: none;"></div>
  </div>
`;
        ...
        const undergradPrograms = programs.slice(0, 3); // Pehle 3
        const masterPrograms = programs.slice(3, 8);    // Next 5

        // Undergraduate Programs
        undergradPrograms.forEach(program => {
          undergradContainer.innerHTML += `<div class="program-item">${program}</div>`;
        });
...
`;
        ...
        masterPrograms.forEach(program => {
          masterContainer.innerHTML += `<div class="program-item">${program}</div>`;
        });
...
`;
        ...
        // Ingénieur Polytechnicien Program (New Accordion)
        polyContainer.innerHTML += `
  <div class="program-item">Ingénieur Polytechnicien Program</div>
  <div style="margin-top:10px; text-align: center;">
    <button style="padding: 12px 20px; font-size: 16px; font-weight:bold; background-color: #5d1049; color: white; border: none; border-radius: 5px; cursor: pointer; transition: background-color 0.3s;" 
      onmouseover="this.style.backgroundColor='#f4c542';" 
      onmouseout="this.style.backgroundColor='#5d1049';" 
...
`;
        ...
        document.getElementById("polyHeading").addEventListener('click', () => {
          toggleAccordion('polyContainer', 'polyIcon');
        });
